...
def retrieve_knowledge(query: str) -> Tuple[str, List[Document]]:
    ...
    对于需要对比多个主题的复杂问题，可以多次调用本工具，每次使用针对性的关键词。

    Args:
        query: 检索关键词。使用简练的关键词而非完整问句，能获得更好的检索效果。
...
def format_docs(docs: List[Document]) -> str:
    ...
    for i, doc in enumerate(docs, 1):
        ...
        chunk_total = metadata.get("_chunk_total")
        position = f"第 {chunk_index}/{chunk_total} 段" if (chunk_index and chunk_total) else ""

        # 构建格式化文本
        formatted = f"【参考资料 {i}】"
        if position:
            formatted += f"（{position}）"
        if header_str:
            formatted += f"\n标题: {header_str}"
        formatted += f"\n来源: {source}"
        formatted += f"\n内容:\n{doc.page_content}\n"
